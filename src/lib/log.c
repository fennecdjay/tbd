...
/* Note: We cannot use the `assert` family in this file due to the risk of infinite recursion */
...
static size_t log_fmt_parse(const size_t dst_size, char *dst,
                            const size_t src_size, const char *src)
{
    ...
    for (size_t dst_i = 0; dst_i < dst_size && src_i < src_size; dst_i++, src_i++)
    {
        if (src[src_i] == '$' && src[src_i + 1] == '{')
        {
            ...
            static const struct text_attr attrs[] = {
                { "reset",     "\033[0m"  },
                { "bold",      "\033[1m"  },
                { "dim",       "\033[2m"  },
                { "italic",    "\033[3m"  },
...
static size_t internal_prettyvlog(FILE *f, const char *fmt, va_list args)
{
    assert(f != NULL);
    assert(fmt != NULL);
...
static size_t internal_prettylog(FILE *f, const char *fmt, ...)
{
    assert(f != NULL);
    assert(fmt != NULL);
...
void vlog(const enum log_type type, const char *fmt, va_list args, const bool newline)
{
    ...
    static const struct text_attr prefix[] = {
        [LOG_NONE]    = { "",            ""                          },
...
static void internal_log(const enum log_type type, const char *fmt, ...)
{
    assert(fmt != NULL);
